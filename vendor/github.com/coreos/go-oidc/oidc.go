...
type UserInfo struct {
	...
	// tdlabs extensions for keycloak
...
// Logout performs a global logout for the current user.
func (p *Provider) Logout(ctx context.Context, tokenSource oauth2.TokenSource, rawIdToken string, postLogoutRedirectURI string, state string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("oidc: process logout response: %v", err)
	}
	...
	return fmt.Sprint(resp.Location()), nil
}
